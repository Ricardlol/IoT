...
test_gc <- cor.test(historic$`glucose levels (mg/dl)`, historic$`blood presure (mm/Hg)`)
print(test_gc)
...
X <- mean(historic$`blood presure (mm/Hg)`)
sd <- sd(historic$`blood presure (mm/Hg)`)
std_error <- sd / sqrt(n)
margin_error <- t_score * std_error
