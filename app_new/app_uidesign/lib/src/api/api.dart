...
class Api {
  ...
  Future<User> getCurrentUser() async {
    try {
      const finalUrl = "${Strings.BASE_URL}users/me";
  ...
  Future<Sensor> updateSensorData(Sensor sensor) async {
    try {
      var finalUrl = "${Strings.BASE_URL}sensors/${sensor.id}";
  ...
  Future<String> login(String phone_number, String password) async {
    try {
      SharedApi.saveServerUri(Strings.BASE_URL);

      print("Login: $phone_number $password");
      const _serverUrl = "${Strings.BASE_URL}token";
